...
//        sceneView.scene.rootNode.addChildNode(node) //Adding the node to the sceen as chlid.
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        configuration.planeDetection = .horizontal //enables horezontal plane detection.
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        if let touch = touches.first {
            ...
            // using the !result.isempty. Whwn using the ! in front the revere is the one used - so in this case is NOT empty.
